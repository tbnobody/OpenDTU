...
DisplayEPaperClass::DisplayEPaperClass()
    : _loopTask(TASK_IMMEDIATE, TASK_FOREVER, std::bind(&DisplayEPaperClass::loop, this))
{
}
...
void DisplayEPaperClass::init(Scheduler& scheduler, DisplayType_t type, uint8_t _CS, uint8_t _DC, uint8_t _RST, uint8_t _BUSY, uint8_t _SCK, uint8_t _MOSI)
{
...
//***************************************************************************
void DisplayEPaperClass::loop()
{
    float totalPower = Datastore.getTotalAcPowerEnabled();
    float totalYieldDay = Datastore.getTotalAcYieldDayEnabled();
    float totalYieldTotal = Datastore.getTotalAcYieldTotalEnabled();
    bool isProducing = Datastore.getIsAtLeastOneProducing();

    // check if the IP has changed
    if (_settedIP != NetworkSettings.localIP().toString()) {
        // save the new IP and call the Headline Funktion to adapt the Headline
        _settedIP = NetworkSettings.localIP().toString();
        headlineIP();
    }

    // call the PowerPage to change the PV Power Values
    actualPowerPaged(totalPower, totalYieldDay, totalYieldTotal, isProducing);

    // if there was an change and the Inverter is producing set a new Timestamp in the footline
    if (isProducing && _changed) {
        _changed = false;
        lastUpdatePaged();
    }
