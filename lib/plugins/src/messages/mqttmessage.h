...
public:
  ...
  void setMqtt(const char *ttopic, const uint8_t *data, size_t len) {
    length = len;
    topic = std::shared_ptr<char[]>(new char[strlen(ttopic) + 1],
                                    std::default_delete<char[]>());
    strcpy(topic.get(), ttopic);
    // hack
    payload = std::shared_ptr<uint8_t[]>(new uint8_t[len],
                                         std::default_delete<uint8_t[]>());
    memcpy(payload.get(), data, len);
  }

  std::shared_ptr<char[]> payloadToChar() {
    auto s = std::shared_ptr<char[]>(new char[length + 1],
                                     std::default_delete<char[]>());
    s[length] = '\0';
  ...
  int toString(char *buffer) {
    ...
    c = c + sprintf(buffer + c, " topic=%s, data=%s, append=%d}", topic.get(),
                    payloadToChar().get(), appendTopic);
    return c;
  }
  virtual const char *getMessageTypeString() { return "MqttMessage"; }
};
